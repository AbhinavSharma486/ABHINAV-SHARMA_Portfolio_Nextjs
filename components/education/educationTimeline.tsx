...
const TimelineItem: React.FC<EducationItem & { idx: number; }> = (props) => {
  ...
  return (
    <div className="relative flex w-full justify-center items-center mb-16 min-h-[100px] md:justify-between md:items-center md:mb-24 md:min-h-[140px]">
        ...
        {isLeft && (
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="font-bold text-xl text-blue-900 dark:text-white">{props.degree}</span>
            </div>
            <div className="font-semibold text-violet-700 dark:text-violet-300 mb-1">{props.institute}</div>
            <div className="text-gray-700 dark:text-gray-300 text-sm mb-2">{props.duration}</div>
            <div className="mt-2 text-gray-600 dark:text-gray-200 text-base">{props.specialization}</div>
            <div className="mt-2 text-gray-600 dark:text-gray-200 text-base">{props.score}</div>
          </motion.div>
        )}
      </div>
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          <span className="text-2xl">💼</span>
          <span className="font-bold text-lg text-blue-900 dark:text-white">{props.degree}</span>
        </div>
        <div className="font-semibold text-violet-700 dark:text-violet-300 mb-1 text-base">{props.institute}</div>
        <div className="text-gray-700 dark:text-gray-300 text-xs mb-2">{props.duration}</div>
        <div className="mt-2 text-gray-600 dark:text-gray-200 text-sm">{props.specialization}</div>
        <div className="mt-2 text-gray-600 dark:text-gray-200 text-sm">{props.score}</div>
      </motion.div>
    </div>
  );
};
...
export const EducationTimeline: React.FC<EducationTimelineProps> = ({ education }) => {
  ...
  return (
    <section id="education" className="py-8 min-h-screen bg-gradient-to-br from-[#f8fafc] via-[#f3e8ff] to-[#e0e7ff] dark:from-[#18181b] dark:via-[#312e81] dark:to-[#0f172a] relative">
      ...
      <div className="container mx-auto px-2 md:px-10 lg:px-20 xl:px-32 relative z-10">
        <h1 className="font-orbitron text-center font-bold dark:text-white text-3xl md:text-5xl mt-4 text-blue-950 mb-2 md:mb-4 drop-shadow-lg">Education</h1>
        ...
        <div className="relative flex flex-col items-center" ref={ref}>
          ...
          <div className="w-full flex flex-col gap-y-10 md:gap-y-0">
            {education.map((edu, idx) => (
              <TimelineItem key={idx} idx={idx} {...edu} />
            ))}
          </div>
        </div>
      </div>
